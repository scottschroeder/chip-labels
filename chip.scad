...
module chip_2d_fit(keep_aspect_ratio = true) {
  chip();
...
module text_chip(s, font = "Noto Sans:style=Bold") {
  chip_2d_fit(keep_aspect_ratio = false)
    text(s,
        halign="center",
        valign="center",
        font = font
...
module resistor_chip() {
  chip_2d()
    scale(.07)
    import("resistor.svg", center=true);
}

  translate([-25, 40,0])
  chip_2d()
scale(.06)
  import("ice-cream-cone-svgrepo-com.svg", center=true);

translate([-25, 20,0])
  resistor_chip();

  chips = ["", "\u03a9", "123457", "M8", "1M\u03a9"];
